...
class FloraSelectionAdapter @Inject constructor(): RecyclerView.Adapter<FloraSelectionAdapter.MyViewHolder>(), Filterable {
    ...
    private var offSeasonSelected= false
    fun setAdapterData(list: List<Flora>,offSeasonSelect: Boolean = false) {
        this.floraList = list
        this.searchedFloraList = list
        this.offSeasonSelected= offSeasonSelect
...
//        if (offSeasonSelected && flora.code == "F999") holder.itemView.appCompatCheckBox.isEnabled = false
        ...
        holder.itemView.constraintLayout.setOnClickListener {
            ...
            if ( flora.code =="F999"){
                ...
                if (!selectedItems.contains(item)) {
                    selectedItems.clear()
                    offSeasonSelected= true
                    selectedItems.add(flora)
                } else {
                    offSeasonSelected= false
                    selectedItems.remove(item)
                }
                notifyDataSetChanged()
            }else{
               if (offSeasonSelected){
