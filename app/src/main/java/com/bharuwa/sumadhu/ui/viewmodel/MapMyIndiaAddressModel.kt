...
import com.bharuwa.sumadhu.network.base.RetrofitMapMyIndia
...
@HiltViewModel
class MapMyIndiaAddressModel @Inject constructor(@RetrofitMapMyIndia private val apiService: ApiEndPoint) : ViewModel() {

    fun getAddress(latitude: Double, longitude: Double) = liveData(Dispatchers.IO) {
        emit(ApiRequest.loading(data = null))
        try {
            val response =apiService.getAddress(latitude,longitude)
